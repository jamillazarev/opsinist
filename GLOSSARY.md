...
| **the reach gate** · **the guard** | the guard runs **at the commit** and refuses what is being committed. **The reach gate runs at the END of a session** and refuses the *ending* — because every other gate here is enforced at a commit, so a run that stops short of one escapes all of them. It is the only gate that fires when nothing is being committed, which is exactly the condition it exists for | is it stopping *this commit*, or stopping *stopping*? |
| **a door** (a request kind) · **a door** (a shipped file) · **a door** (a branch on a diagram) | three unrelated senses, and the third is the one this system's own tooling prints. In the core, *the owner takes it by hand* is **one of the three doors** a request can leave by. In `_ops/scripts/`, **the doors are files** — `transition.py` and `new-id.py`, copied in beside the guard, and what `migrate-layout.py --doors-only` puts back. On a diagram, a *door* is any branch a route may leave through. **A refusal saying your door is missing means the file**, always | is it a *way out of a request*, a *file on disk*, or *an edge on a picture*? |
| **rung** (the evidence scale) · **rung** (a step in a decision ladder) | the evidence rung is *measured › cited › recalled › judgement* — one scale for claims about the world. **A rung in a ladder is something else entirely**: a step you must pass before the next, as in the tool-choosing ladder (*does it need to exist · already here · standard library · native · one line*). The guard's phrase *"the rung above choosing one"* is the second sense, and asks what the work did **before** this tool | is it *how well do we know this*, or *what had to be true first*? |
