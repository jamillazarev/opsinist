...
| **a form** · **a rule** | a rule is a sentence someone must follow; **a form is a shape that fails without them** — a field a liar cannot fill cheaply, a template hole that is visibly empty when skipped, a check that refuses, a writer restriction. The distinction is the whole of this system's measured practice: rules stated in always-loaded prose scored **0/5, 0/5 and 0/3** in one release, and two of them rebuilt as refusals went **5/5** each (2026-08, mid tier). So *"this needs a form"* never means *say it more firmly* — it means *find the artifact where the omission shows* | if nobody reads the sentence, does anything still stop? |
