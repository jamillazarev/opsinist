...
| **taking a repo over** | `N8` 0/2 → 0/5 → 1/5 → **4/5 · 4/5** | **The one row that moved, and the diagnosis here was wrong.** `N8` did not *refuse* to audit — it could not **reach** `entering.md`, because the core cited its companions by bare name and a run resolved them against the skill's own directory. A path line, a `join` door and **two hooks that travel with the plugin**: one refusing a mutating call before the debt list exists, one stopping a run that presented deferrable findings and wrote no `LATER.md`. Now **5/5 audit before touching** and `LATER.md` lands in **5 of 5**, against 1 of 5 (`RUNS.md`, `2026-08-01`). What the hooks cannot hold — *did the owner say yes* — is named rather than faked |
| **import mapping** | `N28` all void → all void | **untested** — void by my own dispatch-sheet defect, in both rounds |
