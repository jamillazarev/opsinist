...
255. **A deferral is kept honest by its trigger, and a trigger has three ways to fail — only one of which the deferral itself can see.** It can be absent, which every list already guards against. It can be **unreachable**: a condition naming something that does not exist, which reads as diligence and never returns — measured 2026-09-06, an entry waiting a fortnight on a file this repository does not have and by its own rules should not. And it can have **already fired** while nobody looked: the same audit found an entry holding open for a measurement that had run, whose answer was the opposite of what the entry hoped. **So a deferred list is read against reality on a cadence, not against its own prose** — an entry that reads well is exactly the kind that is never re-read.
