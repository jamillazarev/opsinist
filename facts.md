...
259. **A research answer that does not say how hard anyone looked reads as if someone looked hard** — and this one is a judgement call, not a measurement, which is worth saying in the file that other writing quotes. Depth is invisible in a finished document: the same page can be twenty minutes or six hours and the reader cannot tell, so naming the rung before the work is offered as a price the owner agrees to. **Three rungs named by what they buy** — a direction, a defensible call, or something quotable in a year. **Nothing here has measured whether stating it changes a run**, and the first draft of this line shipped as an assertion with a date attached to the wrong thing; the date belonged to a runtime check, not to a defect anyone observed.
