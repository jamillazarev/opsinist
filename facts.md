...
249. **More documentation, iterated without a human curating it, plateaus and then hurts.** Measured elsewhere and worth having: runs latch onto different parts of a grown document or ignore some of it, and the rate stops improving while the text keeps growing. The instinct when a run gets something wrong is to explain more; the finding is that explaining more is the move that stops working first, and that what a document needs is someone deciding what comes OUT. This is the noise rule the corpus already applies to checkers and to migrated backlogs, arriving at the documents themselves.

250. **An interactive multi-step flow is a form that does not hold.** A run walked through a configuration wizard abandons it partway — measured by a team that built one and watched it happen. A pre-written chunk of the thing the wizard would have produced held at 93–100% on the stronger tiers. So the ladder of forms has a rung below the ones already named: a refusal holds · a template hole holds · **a conversation the run must complete does not**, because nothing forces the last step and the run is free to leave at any of them.

251. **The same document produces wildly different rates by tier, and improving it does not close the gap.** One team's before-and-after, on their own design-system docs (2026, their measurement, their models): 3% → 47% on their weakest tier, 20% → 90% in the middle, 40% → 100% at the top. Every band improved and the floor stayed a floor. So a document is not "good" or "bad" — it is good *for a tier*, and a rewrite that triples the weak tier can still leave it failing half the time. **State the tier with the rate, always**, and do not price work at the top band's number.

252. Whether a document WORKS is measurable, and measuring whether a rule holds does not measure it. A rule is measured by running the situation and counting the outcome; a document is measured by giving several runs the same task with it and counting what came back — tries to a clean result · violations of the standard it teaches · size of what was produced · how far each iteration moved · what the reading cost. The two ask different questions: a rule can hold in a document nobody can use, and a document can produce good work while the rule inside it is never reached. **And the counts say where, never why** — the team that ran them said the feedback showed the smoke and not the fire. Borrowed 2026-08-23; **not yet run here**, which is the honest state of it.
