...
259. **A research answer that does not say how hard anyone looked reads as if someone looked hard.** Depth is invisible in a finished document — the same page can be twenty minutes or six hours, and the reader cannot tell — so it is chosen **before** the fetching starts, stated in the answer, and priced when it is offered, because how long a search takes is a spend and spending is the owner's decision. Three rungs, named by what they buy rather than by effort: a **direction**, a **defensible call**, or **something quotable in a year** — and only the last one obliges every source to have been read against the others, which is why it is the only rung worth saying no to. **A run that outgrows its rung is a new question at a new price**, never a quiet overrun.
