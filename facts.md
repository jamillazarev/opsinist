...
261. **A checker's false warnings are not noise around the signal — they are where the signal hides.** Measured 2026-09-10 on this repository's own structure check: **thirteen warnings, eight of them false by construction** — seven comparing a dated changelog entry's count against a corpus that had moved on, which can only ever drift further, and one charging a sentence with a list that was not its own — and **three of the five real ones had already shipped a release**: a table row wrapped onto a second source line, a paragraph whose line broke before `5.` so markdown read an ordered list, a hyphenated word split by a line break. Each was one line long and none was hard; what made them invisible was standing tenth in a list nobody finished reading. **The repair is to stop making the comparison that cannot be won**, not to raise the threshold or to teach anyone to skim past the known ones — a permanent false warning is a check that has been switched off by everyone who reads it, while still reporting.
