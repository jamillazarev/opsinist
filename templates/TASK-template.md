...
|---|---|
| {{the thing}} | {{the path, the register, the page}} |

**Check** (the mechanical half, run clean *before* review is asked for): {{command, or none}}

> **A deliverable with no destination is how work is declared done and stays unfindable**, and the
> list is checked as a list — each named thing at its named place.

## History

- {{date}} — {{what ran, what it produced}} · run {{R-XXXXXX}}
