...
a count that drifts is reported as a warning, never a failure (`preflight.sh` §10 renders all of
`check-structure`'s output as warnings) — **but the trio's own counts are deltas, and are not**:
