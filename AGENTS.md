...
a count that drifts is reported as a warning, never a failure (`preflight.sh` §10 renders every line
`check-structure` prints that way) — **but the trio's own counts are deltas, and are not**:
