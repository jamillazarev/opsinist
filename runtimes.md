...
| **deferred tool schemas** | a wired server costs a line in a list until something calls it | **every schema is loaded before the first word**, so the tools are paid for on a session that uses none of them — forty of them is tens of thousands of tokens. The remedy is the register's, not a wrapper's: wire fewer servers, and let the ones that stay earn their weight. Observed on this runtime 2026-08-23, where tools arrive as names and a schema is fetched when one is wanted; **state the runtime and the date, because this is moving** |
